...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests()
                ...
                .antMatchers("/manager/**").access("hasRole('ROLE_ADMIN') or hasRole('ROLE_MANAGER')") //! 매니저 접근은 매니저나 어드민의 역할을 가져야만 접근가능
...
//                .antMatchers("/").permitAll()
                ...
                .and()
                .formLogin()
                .loginPage("/loginForm") //! user, admin, manager의 역할이 아예없는 권한이 없는 친구들은 전부 로그인 페이지로 보냄
